...
export const ExternalApiComponent = () => {
  const { apiOrigin = "https://7v4wg8hxb2.execute-api.us-west-2.amazonaws.com/prod2/api/v1", audience } = getConfig();
  ...
  const callApi = async () => {
    try {
      ...
      const response = await fetch(`${apiOrigin}/member`, {
        headers: {
          Authorization: `${token}`,
        },
      });
      ...
      setState({
        ...state,
        showResult: true,
        apiMessage: responseData.body.data,
      });
    } catch (error) {
      setState({
        ...state,
